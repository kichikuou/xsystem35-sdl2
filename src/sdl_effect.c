...
static EffectTexture *create_effect_texture(agsurface_t *as, int x, int y, int w, int h) {
	...
	if (!as) {
		...
		t->tx = SDL_CreateTextureFromSurface(sdl_renderer, sdl_dib);
	} else {
...
static void effect_texture_free(EffectTexture *t) {
	if (!t)
		return;
...
static void effect_finish(struct sdl_effect *eff, bool present) {
	...
	effect_texture_free(eff->tx_old);
	effect_texture_free(eff->tx_new);
}
...
static void linear_blur_free(struct sdl_effect *eff) {
	...
	for (int i = 0; i < LINEAR_BLUR_STEPS; i++) {
		if (this->blurred_old[i])
			effect_texture_free(this->blurred_old[i]);
		if (this->blurred_new[i])
			effect_texture_free(this->blurred_new[i]);
	}
	effect_finish(&this->eff, true);
	free(this);
}
